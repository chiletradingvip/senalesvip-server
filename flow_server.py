...
def crear_orden_flow(nombre, email, telegram, telefono, monto, order_id):
    params = {
        ...
        "urlConfirmation":  f"{NGROK_URL}/flow/notificar",
        "urlReturn":        f"{NGROK_URL}/flow/retorno",
...
def iniciar_pago():
    try:
        ...
        url_pago = crear_orden_flow(nombre, email, telegram, telefono, 350, order_id)
        if url_pago:
...
@app.route('/flow/notificar', methods=['POST'])
def notificar_pago():
    """Flow llama aquí en background para confirmar el pago"""
    try:
...
@app.route('/flow/retorno', methods=['GET', 'POST'])
def retorno_pago():
    """Flow redirige al cliente aquí después del pago"""
    return redirect(f"{PAGINA_URL}?pago=exitoso")

@app.route('/flow/confirmar', methods=['GET', 'POST'])
def confirmar_pago():
    """Endpoint legacy - redirige a página de éxito"""
    return redirect(f"{PAGINA_URL}?pago=exitoso")

@app.route('/health', methods=['GET'])
def health():
    return jsonify({"status": "running", "endpoints": ["/flow/iniciar", "/flow/notificar", "/flow/retorno"]}), 200

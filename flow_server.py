...
# ============================================
try:
    from config_flow import FLOW_API_KEY, FLOW_SECRET_KEY, FLOW_URL
except:
...
@app.route('/flow/confirmar', methods=['POST'])
def confirmar_pago():
    try:
        ...
        if pago.get('status') == 2:
            ...
            monto    = int(pago.get('amount', 25000))
            email    = pago.get('payer', '')
            opcional = json.loads(pago.get('optional', '{}'))
